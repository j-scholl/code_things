...
// String.valueOf(temp.charAt(i)).equals(String.valueOf(secretvalue.charAt(j)))
    // Methods
        ...
        for (int i = 0; i < temp.length(); i++) {
            for (int j = 0; j < secretvalue.length(); j++) {
                if ((temp.charAt(i)) == ((secretvalue.charAt(j)))) {   // if its the same value for one of the values...
                    if (i != j) {
                        littleMoo++;     // ...and if its NOT in the same place as that value... increment littleMoo.
                    }
                }
            }
        }
        int littleTruMoo = littleMoo - getBigMooCount(guess);
        return littleTruMoo;
    }
    ...
    public boolean isCorrectGuess(int guess){
        String temp = String.valueOf(guess);
        for (int i = 0; i < 4; i++) {
            if (!(String.valueOf(temp.charAt(i)).equals(String.valueOf(secretvalue.charAt(i))))){
                return false;
            }
        }
        return true;
